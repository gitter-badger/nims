...
class Subject(DataContainer):
    ...
    def from_mrfile(cls, mrfile):
        subj_code, group_name, exp_name = nimsutil.parse_patient_id__(mrfile.subj_code, mrfile.group_name, mrfile.experiment_name, ResearchGroup.all_ids())
...
class Session(DataContainer):
    ...
    def from_mrfile(cls, mrfile):
        ...
        if not session:
            ...
            session = Session(uid=uid, exam=int(mrfile.exam_no) if mrfile.exam_no.isdigit() else 0, subject=subject, operator=operator)
        return session
